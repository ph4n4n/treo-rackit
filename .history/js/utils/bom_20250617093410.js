class BOM {
    ...
    renderBOM(counts) {
        ...
        if (counts['pipe-segment'].size > 0) {
            ...
            title.innerHTML = '<i class="bi bi-dash-lg me-2"></i>Ống thép';
            ...
            Array.from(counts['pipe-segment'].entries())
                ...
                .forEach(([length, count]) => {
                    ...
                    details.innerHTML = `
                        <div class="bom-item-name">Ống ${length}cm</div>
                        <div class="bom-item-specs text-muted small">Đường kính 25mm</div>
                    `;
        ...
        const otherComponents = [
            { type: 'elbow-joint', label: 'Co 90°', icon: 'bi-corner-up-right', specs: 'Đường kính 25mm' },
            { type: 'tee-joint', label: 'Khớp T', icon: 'bi-plus', specs: 'Đường kính 25mm' },
            { type: 'wall-mount', label: 'Chân đế', icon: 'bi-house', specs: 'Tải trọng 50kg' }
        ];

        otherComponents.forEach(({ type, label, icon, specs }) => {
            if (counts[type] > 0) {
                ...
                const title = document.createElement('h6');
                title.className = 'text-primary mb-2';
                title.innerHTML = `<i class="${icon} me-2"></i>${label}`;
                ...
                const details = document.createElement('div');
                details.innerHTML = `
                    <div class="bom-item-name">${label}</div>
                    <div class="bom-item-specs text-muted small">${specs}</div>
                `;
        ...
        if (totalItems === 0) {
            ...
            emptyState.innerHTML = `
                ...
                <small>Chưa có linh kiện nào<br>Kéo thả hoặc click để thêm</small>
            `;
