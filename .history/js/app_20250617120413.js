class TreoRackitApp {
    ...
    setupSidebar() {
        ...
        // Phân nhóm components theo category
        const categorizedComponents = {
            pipes: ['pipe-segment'],
            joints: ['elbow-joint', 'tee-joint'],
            mounts: ['wall-mount']
        };
        
        // Thêm components vào từng category
        Object.entries(categorizedComponents).forEach(([category, componentIds]) => {
            const categoryGrid = document.querySelector(`[data-category="${category}"] .component-grid`);
        ...
        document.querySelectorAll('.category-header').forEach(header => {
            header.addEventListener('click', () => {
                const category = header.closest('.component-category');
                const content = category.querySelector('.category-content');
                const icon = header.querySelector('.category-icon');
                
                content.style.display = content.style.display === 'none' ? 'block' : 'none';
                icon.style.transform = content.style.display === 'none' ? 'rotate(-90deg)' : 'rotate(0deg)';
                category.classList.toggle('collapsed', content.style.display === 'none');
            });
        });
    }
